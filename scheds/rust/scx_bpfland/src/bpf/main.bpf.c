...
static u64 task_deadline(struct task_struct *p, struct task_ctx *tctx)
{
	...
	/*
	 * We want to prioritize producers (wakers) more than consumers
	 * (blocked): this seems to improve responsiveness with
	...
	avg_run_raw = MIN(avg_run_raw, slice_lag);
	avg_run_raw = scale_inverse_fair(p, tctx, avg_run_raw);
	...
	lat_scale = MIN(lat_scale, MAX_TASK_WEIGHT);

	return tctx->avg_runtime * 100 / lat_scale;
}
